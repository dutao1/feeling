...
CREATE TABLE `event_base_info` (
  ...
  `event_city` varchar(32) COLLATE utf8_bin NOT NULL COMMENT '发布事件城市名称',
  `lat` float NOT NULL DEFAULT '0' COMMENT '发布事件的纬度',
  `lon` float NOT NULL DEFAULT '0' COMMENT '发布事件的经度',
  `location_long_code` bigint(8) NOT NULL DEFAULT '0' COMMENT '长整形数字坐标',
  `is_display` tinyint(2) NOT NULL DEFAULT '1' COMMENT '0:不显示  1：显示',
  `spread_times` int(4) NOT NULL DEFAULT '0' COMMENT '被传播次数',
  `skip_times` int(4) NOT NULL DEFAULT '0' COMMENT '被忽略次数(超50次不再推荐)',
  `comment_times` int(4) NOT NULL DEFAULT '0' COMMENT '被评论次数',
...
CREATE TABLE `event_comment_record` (
  ...
  `eid` int(11) DEFAULT NULL COMMENT '事件id',
  ...
  `create_time` datetime DEFAULT '1997-01-01 00:00:00' COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `event_cycle_record` (
  ...
  `eid` int(11) DEFAULT NULL COMMENT '原始事件id',
  `location_hash` varchar(32) COLLATE utf8_bin DEFAULT NULL COMMENT '经纬度hash值',
  PRIMARY KEY (`id`),
...
CREATE TABLE `event_pic` (
  ...
  `remark` varchar(64) NOT NULL COMMENT '图片说明',
  `create_time` datetime NOT NULL COMMENT '创建时间',
...
CREATE TABLE `event_vote` (
  ...
  `votes1` int(4) NOT NULL DEFAULT '0' COMMENT '选项1得票数',
  `votes2` int(4) NOT NULL DEFAULT '0' COMMENT '选项2得票数',
  `votes3` int(4) NOT NULL DEFAULT '0' COMMENT '选项3得票数',
  `votes4` int(4) NOT NULL DEFAULT '0' COMMENT '选项4得票数',
  `votes5` int(4) NOT NULL DEFAULT '0' COMMENT '选项5得票数',
  `votes6` int(4) NOT NULL DEFAULT '0' COMMENT '选项6得票数',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`eid`),
  KEY `i_uid` (`uid`)
...
CREATE TABLE `user_base_info` (
  ...
  `nick_name` varchar(32) NOT NULL COMMENT '昵称(唯一且不能修改)',
  ...
  `status` tinyint(2) NOT NULL DEFAULT '0' COMMENT '-9: 注销 0:正常 1:锁定',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `update_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`uid`),
  KEY `mobile` (`mobile`)
